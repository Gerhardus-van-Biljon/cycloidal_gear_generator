...
- Added optional "Balance Disk": a second cycloid disk mounted 180 degrees out of phase
  ...
  * Exported to its own DXF layers (CYCLOID_DISK_2, OUTPUT_HOLES_2, ECCENTRIC_CAM_2) and to
    SVG with dashed strokes so it's visually distinguishable from disk 1.
'''
...
def export_to_dxf(filename, params, phi=0):
    ...
    - Optional balance disk (180 deg phase-offset second disk) exported to its own layers.
    """
    try:
        import ezdxf
    except ImportError:
    ...
    show_balance = params.get("show_balance_disk", False)
    ...
    layers_config = [
        ...
        ("CENTER_AXIS", 4),         # Cyan
        ("CYCLOID_DISK_2", 30),     # Orange (Balance disk)
        ("OUTPUT_HOLES_2", 40),     # Balance disk output holes
        ("ECCENTRIC_CAM_2", 50),    # Balance disk eccentric lobe
    ]
    ...
    #  BALANCE DISK (180 deg phase-offset second disk) 
    if show_balance:
        # phi2 = phi + pi drives BOTH the eccentric translation and the lobe rotation,
        # which keeps the disk correctly meshed with the SAME ring pins (it's the same
        # rolling family evaluated at a shifted crank angle), while landing the eccentric
        # mass on the opposite side of the shaft for balance.
        phi2 = phi + np.pi
        disk2_center_x = e * np.cos(phi2)
        disk2_center_y = e * np.sin(phi2)

        # Output holes: the disk body itself picks up an extra -pi/num_lobes of rotation
        # from using phi2 instead of phi, so we add +pi/num_lobes of local phase to the
        # hole pattern to cancel that out and land back on the SAME OUTPUT_PINS layer.
        hole_phase_offset = np.pi / num_lobes
        for i in range(No):
            a = 2 * np.pi * i / No + hole_phase_offset
            cx = Rd * np.cos(a) + disk2_center_x
            cy = Rd * np.sin(a) + disk2_center_y
            msp.add_circle((cx, cy), hole_r, dxfattribs={"layer": "OUTPUT_HOLES_2"})

        # Eccentric cam lobe, mirrored to the opposite side of the shaft
        if cam_lobe_r > 0:
            msp.add_circle((disk2_center_x, disk2_center_y), cam_lobe_r, dxfattribs={"layer": "ECCENTRIC_CAM_2"})

        # Cycloid disk profile 2 - reuses the same (phi-independent) xd, yd shape,
        # just rotated/translated using phi2 instead of phi.
        x2 = xd*np.cos(-phi2/num_lobes) - yd*np.sin(-phi2/num_lobes) + disk2_center_x
        y2 = xd*np.sin(-phi2/num_lobes) + yd*np.cos(-phi2/num_lobes) + disk2_center_y

        spline_points2 = [(float(x2[i]), float(y2[i]), 0) for i in range(len(x2))]
        disk_spline2 = msp.add_spline(spline_points2, dxfattribs={"layer": "CYCLOID_DISK_2"})
        disk_spline2.closed = True
...
def export_to_svg(filename, params, phi=0):
    ...
    if show_balance_disk:
        ...
        ic2 = inner_circles(eccentricity, num_output_pins, num_external_pins, output_pin_diameter, output_disk_diameter, phi2, tolerance, hole_phase_offset)
        for circle in ic2:
            path = points_to_path(circle)
            svg_lines.append(f'<path d="{path}" fill="none" stroke="#AA44FF" stroke-width="0.5" stroke-dasharray="2,1.2"/>')

        ecc_cam2 = eccentric_camshaft(eccentricity, camshaft_diameter, phi2)
...
def eccentric_camshaft(eccentricity, camshaft_diameter, phi):
    ...
    Call with (phi + np.pi) to get the balance disk's mirrored lobe (180 deg
    opposite the primary disk's lobe on a double-lobe eccentric shaft)."""
    t = np.linspace(0, 2*np.pi, 200)
...
class GearboxViewer(gl.GLViewWidget):
    ...
    def update_geometry(self, eccentricity, num_external_pins, num_output_pins, ring_diameter, pin_diameter, output_disk_diameter, output_pin_diameter, camshaft_diameter, tolerance, show_outer_ring, outer_ring_width, phi, show_balance_disk=False):
        """Update all geometry"""
        # Outer pins
        pins = pin_ring(num_external_pins, ring_diameter, pin_diameter)
...
class SliderPanel(QtWidgets.QWidget):
        ...
        if filename:
            try:
                ...
                if self.params.get('show_balance_disk'):
                    layer_note += (
                        "- CYCLOID_DISK_2 (balance disk)\n"
                        "- OUTPUT_HOLES_2 (balance disk)\n"
                        "- ECCENTRIC_CAM_2 (balance disk)\n"
                    )
    ...
    def reset_params(self):
        ...
        self.balance_disk_checkbox.setChecked(defaults['show_balance_disk'])
