...
class SliderPanel(QtWidgets.QWidget):
    ...
    def export_dxf(self):
        ...
        if filename:
            try:
                export_to_dxf(filename, self.params, -np.pi/2, disk_variant="primary")
                exported_files = [filename]

                if self.params.get('show_balance_disk'):
                    balance_filename = self._make_balance_filename(filename)
                    export_to_dxf(balance_filename, self.params, -np.pi/2, disk_variant="balance")
    ...
    def export_svg(self):
        ...
        if filename:
            try:
                export_to_svg(filename, self.params, -np.pi/2)
                QtWidgets.QMessageBox.information(
                    self,
                    "Export Successful",
