...
def inner_circles(eccentricity, num_output_pins, num_external_pins, output_pin_diameter, output_disk_diameter, phi):
    """output_pin_diameter and output_disk_diameter are diameters, not radii"""
    t = np.linspace(0, 2*np.pi, 200)
    num_lobes = num_external_pins - 1  # Number of lobes = num_external_pins - 1
    circles = []
    ...
    for i in range(num_output_pins):
        ...
        # hole_radius = output_pin_radius (pin radius) + eccentricity (eccentricity clearance)
        hole_radius = output_pin_radius + eccentricity
...
def cycloid_disk(eccentricity, num_external_pins, ring_diameter, pin_diameter, phi):
    ring_radius = ring_diameter/2  # Convert to radius
    pin_radius = pin_diameter/2  # Convert to radius
    num_lobes = num_external_pins - 1  # Number of lobes = num_external_pins - 1
    t1 = np.linspace(-np.pi/num_lobes, np.pi/num_lobes, 1500)
    ...
    for i in range(num_lobes):
        xa = (rolling_circle_radius + stationary_circle_radius)*np.cos(t1) - eccentricity*np.cos((rolling_circle_radius + stationary_circle_radius)/stationary_circle_radius*t1)
        ya = (rolling_circle_radius + stationary_circle_radius)*np.sin(t1) - eccentricity*np.sin((rolling_circle_radius + stationary_circle_radius)/stationary_circle_radius*t1)

        dxa = (rolling_circle_radius + stationary_circle_radius)*(-np.sin(t1) + (eccentricity/stationary_circle_radius)*np.sin((rolling_circle_radius + stationary_circle_radius)/stationary_circle_radius*t1))
        dya = (rolling_circle_radius + stationary_circle_radius)*( np.cos(t1) - (eccentricity/stationary_circle_radius)*np.cos((rolling_circle_radius + stationary_circle_radius)/stationary_circle_radius*t1))

        xd = xa + pin_radius/np.sqrt(dxa**2 + dya**2)*(-dya)
        yd = ya + pin_radius/np.sqrt(dxa**2 + dya**2)*( dxa)

        x = xd*np.cos(i*2*np.pi/num_lobes - phi/num_lobes) - yd*np.sin(i*2*np.pi/num_lobes - phi/num_lobes) + eccentricity*np.cos(phi)
        y = xd*np.sin(i*2*np.pi/num_lobes - phi/num_lobes) + yd*np.cos(i*2*np.pi/num_lobes - phi/num_lobes) + eccentricity*np.sin(phi)

        curves.append(np.vstack([x, y, np.zeros_like(x)]).T)
    return curves


def camshaft(camshaft_diameter, phi):
    """Generate camshaft hole in the cycloid disk - this is the fixed outer boundary"""
    t = np.linspace(0, 2*np.pi, 200)
    # This is the hole in the cycloid disk - uses full camshaft_diameter
    x = camshaft_diameter/2 * np.cos(t)
    y = camshaft_diameter/2 * np.sin(t)
...
class GearboxViewer(gl.GLViewWidget):
    ...
    def rebuild_items(self, num_external_pins, num_output_pins):
        ...
        self.cycloid_items = [gl.GLLinePlotItem(color=(1, 0.3, 0.3, 1), width=2.5) for _ in range(num_external_pins)]
    ...
    def update_geometry(self, eccentricity, num_external_pins, num_output_pins, ring_diameter, pin_diameter, output_disk_diameter, output_pin_diameter, camshaft_diameter, phi):
        """Update all geometry"""
        # Outer pins
        ...
        # Inner circles
        ic = inner_circles(eccentricity, num_output_pins, num_external_pins, output_pin_diameter, output_disk_diameter, phi)
        for i, item in enumerate(self.inner_circle_items):
            if i < len(ic):
                item.setData(pos=ic[i])

        # Cycloid disk
        cd = cycloid_disk(eccentricity, num_external_pins, ring_diameter, pin_diameter, phi)
        for i, item in enumerate(self.cycloid_items):
            if i < len(cd):
                item.setData(pos=cd[i])
        
        # Camshaft (stationary center)
        cam = camshaft(camshaft_diameter, phi)
