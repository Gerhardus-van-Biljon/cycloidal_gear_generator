...
def export_to_dxf(filename, params, phi=0):
    """Optimized DXF export matching simulation visuals exactly"""
    try:
        import ezdxf
    except ImportError:
    ...
    # Calculate offset of the disk (Simulation logic)
    ...
    for i in range(Ne):
        ...
        cx = R * np.cos(a)
        cy = R * np.sin(a)
        msp.add_circle((cx, cy), pin_d / 2, dxfattribs={"layer": "EXTERNAL_PINS"})
    ...
    # ===================== 5. CAMSHAFT HOLE (Static at 0,0) =====================
    # MATCHES SIMULATION: The 'camshaft' function draws this at (0,0)
    # This represents the static center clearance, not the moving hole in the disk.
    cam_hole_r = cam_d / 2 + tol
    msp.add_circle((0, 0), cam_hole_r, dxfattribs={"layer": "CAMSHAFT_HOLE"})

    # ===================== 6. ECCENTRIC CAM (Moves with Disk) =====================
    # MATCHES SIMULATION: The 'eccentric_camshaft' function draws this at (disk_center_x, disk_center_y)
    # Size Calculation: (Diameter - 2*Eccentricity) / 2
    cam_lobe_r = (cam_d - 2 * e) / 2
    
    if cam_lobe_r > 0:
        msp.add_circle((disk_center_x, disk_center_y), cam_lobe_r, dxfattribs={"layer": "ECCENTRIC_CAM"})

    # ===================== 7. OUTER RING (Smoothed) =====================
    if show_ring:
        inner_prof, _ = outer_ring(Ne, ring_d, pin_d, ring_w, tol)
        
        # Inner: Spline
        inner_pts_3d = [(float(p[0]), float(p[1]), 0) for p in inner_prof]
        msp.add_spline(inner_pts_3d, dxfattribs={"layer": "OUTER_RING"})

        # Outer: Circle
        outer_radius = (ring_d / 2) + ring_w
        msp.add_circle((0, 0), outer_radius, dxfattribs={"layer": "OUTER_RING"})

    # ===================== 8. CYCLOID DISK (Spline) =====================
    points_per_lobe = 80 
    num_lobes = Ne - 1
    ...
    # Apply rotation and translation (disk shift)
    ...
    msp.add_spline(spline_points, dxfattribs={"layer": "CYCLOID_DISK"})
